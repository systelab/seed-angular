...
export class MainComponent implements OnInit {
	...
	public doPatientSearch() {
		...
		this.dialogService.showDialog(PatientMaintenanceDialog, parameters).subscribe();
	}
	...
	public doChangeLanguage(language: string) {
		...
		this.i18nService.use(language).subscribe(() => window.location.reload(), error => console.log('Error setting the language.'));
	}
}
