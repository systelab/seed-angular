...
export class GridService {
    ...
    public static getGridHeaderText(grid: ElementFinder, col: number) {
        return GridService.getGridHeader(grid, col).element(by.className('ag-header-cell-text')).getText();
    }
    ...
    public static getNumberOfRows(grid: ElementFinder) {
        return grid.all(by.css('div.ag-body-container div.ag-row')).count();
    }
    ...
    public static checkGridHeaders(grid: ElementFinder, titles: string[]) {
        ...
        GridService.gridHasData(grid)
            .then((data) => {
                if (data === true) {
                    TestUtil.checkIsPresent(GridService.getRow(grid, 0).get(0), 'a Cell');
                } else {
