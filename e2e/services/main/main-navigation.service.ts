...
export class MainNavigationService {
	...
	public static async navigateToPatientMaintenancePage(mainPage: MainPage): Promise<void> {
		await TestUtil.checkPageIsPresentAndDisplayed(mainPage);
		await mainPage.getPatientButton().click();
	}

	public static async navigateToAllergyMaintenancePage(mainPage: MainPage): Promise<void> {
		await TestUtil.checkPageIsPresentAndDisplayed(mainPage);
